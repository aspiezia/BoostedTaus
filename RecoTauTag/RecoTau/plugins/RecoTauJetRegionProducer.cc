...
void RecoTauJetRegionProducer::produce(edm::Event& evt, const edm::EventSetup& es) 
{
  ...
  // std::cout <<" In RecoTauJetRegionProdcer.cc before  edm::Handle<edm::View<reco::PFCandidate>> pfCandsHandle;" <<std::endl; 
  // edm::Handle<edm::View<reco::PFCandidate>> pfCandsHandle; //
  edm::Handle<reco::PFCandidateCollection> pfCandsHandle;
  //std::cout <<" In RecoTauJetRegionProdcer.cc after edm::Handle<edm::View<reco::PFCandidate>> pfCandsHandle; "<< std::endl; 
  evt.getByToken(pf_token, pfCandsHandle);
  //std::cout <<" In RecoTauJetRegionProdcer.cc after evt.getByToken(pf_token, pfCandsHandle); "<< std::endl; 
  ...
  // std::cout <<" In RecoTauJetRegionProdcer.cc after pfCands.reserve(pfCandsHandle->size()); of size "<<  pfCandsHandle->size()<< std::endl; 
  for ( size_t icand = 0; icand < pfCandsHandle->size(); ++icand ) {
    //  std::cout <<" In for cycle " << std::endl;
    pfCands.push_back(PFCandPtr(pfCandsHandle, icand));
  }
  // std::cout <<" In RecoTauJetRegionProdcer.cc after  pfCands.push_back(PFCandPtr(pfCandsHandle, icand));" <<std::endl; 
  // Get the jets
